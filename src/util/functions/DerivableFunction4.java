...
public class DerivableFunction4 extends DerivableFunction {
	...
	public double getValueAt(Matrix x) {
		if (super.memorizedValues.containsKey(x)) {
			...
			double result = Math.pow(x.getValue(0, 0)-3, 2) + Math.pow(x.getValue(1, 0), 2);
			super.callCount++;
	...
	public Matrix getDerivativeAt(Matrix x) {
		if (super.memorizedDerivatives.containsKey(x)) {
			...
			Matrix derivative;
			double dx1 = 2*(x.getValue(0, 0) - 3);
			double dx2 = 2*x.getValue(1, 0);
