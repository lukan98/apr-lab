...
public class NewtonRaphson {
	...
	public NewtonRaphson(double epsilon) {
	...
	public Matrix findMinimum(Matrix startingPoint, DerivableFunction f, boolean optimalSteps, boolean verbose) {
		...
		while(true) {
			...
			Matrix inverseHessian = LUPDecomposition.calculateInverse(f.getHessianMatrix(xs));
			v = inverseHessian;
			v = v.multiply(f.getDerivativeAt(xs));
			v = v.multiply(-1.0);
		...
		if (verbose) {
			System.out.println("Broj iteracija Newton-Raphsonovog postupka: " + iterationCount);
